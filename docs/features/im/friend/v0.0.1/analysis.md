...
实现好友系统的服务端能力与首版 Flutter 客户端：搜索用户、发送好友申请（附留言）、接受/拒绝申请、好友列表、好友资料、发起聊天和删除好友。好友申请和接受通过 WebSocket 实时推送通知，接受好友后自动创建私聊会话并发送打招呼消息。

### 本次客户端实现边界（2026-07-29）

- 设计参照微信通讯录、新的朋友、添加朋友和搜索页的信息层级，但沿用 Flash IM 现有蓝色主题、头像组件与导航结构。
- 只呈现当前服务端已有契约能够闭环的入口：好友列表、新的朋友、用户搜索、好友申请、好友资料、发消息、删除好友。
- 不展示尚无客户端或服务端闭环的微信入口：手机通讯录、扫一扫、雷达、面对面建群、群聊、标签、公众号、服务号、企业联系人、个人二维码。
- 搜索支持服务端现有的昵称、手机号与闪讯号；搜索结果根据 `relation_status` 显示“添加”“等待验证”“接受”或“发消息”。
- 客户端落在独立包 `client/modules/flash_im_friend`，宿主仅负责仓储/WS/Cubit 注入、通讯录 Tab 接线和进入聊天页。
...
A 在通讯录页面点击“+”入口，进入添加朋友页。点击搜索栏进入搜索页，输入 B 的昵称、手机号或闪讯号。搜索结果中点击 B，查看 B 的资料页，点击“添加到通讯录”，填写不超过 200 字的验证留言后发送申请。
...
B 收到 WS 推送的好友申请通知（通讯录 Tab 与“新的朋友”入口出现红点）。B 点击通讯录 Tab，再点击“新的朋友”，看到 A 的申请（头像、昵称、留言与时间）。B 点击“接受”，A 和 B 成为好友，系统自动创建私聊会话并发送打招呼消息。B 的好友列表中出现 A，会话列表中出现与 A 的私聊。
...
A 在好友列表中点击 B 进入资料页，点击删除入口并二次确认。B 从 A 的好友列表中消失，双方的好友关系解除。B 会收到好友删除的 WS 通知，B 的好友列表中 A 也消失。
...
| P-22 | 用户搜索页 | 前端业务 | 搜索用户 + 用户资料页 + 发送好友申请 |
| P-23 | 好友申请通知 | 前端业务 | 收到申请时通讯录 Tab 红点提示 |
| P-24 | 好友详情页 | 前端业务 | 好友资料展示 + 发消息 + 删除好友 |
| P-25 | 添加朋友页 | 前端业务 | 搜索入口与已实现能力说明 |
| P-26 | 陌生人资料页 | 前端业务 | 搜索结果展示 + 添加到通讯录 |
...
- 开发顺序建议：
  ...
  7. 前端 flash_im_friend 模块（好友列表页 + 申请页 + 搜索/资料页）
  8. 将现有“通讯录”占位页替换为好友模块，并接入未处理申请红点
  9. 全链路联调
...
- 暂不实现：
  ...
  - 好友数量限制
  - 手机通讯录、扫一扫、雷达、面对面建群、个人二维码等发现能力
  - 群聊、标签、公众号、服务号、企业联系人等通讯录分类入口
